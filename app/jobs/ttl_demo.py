...
def create_test_content_for_existing_articles():
    ...
    try:
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            existing_content = db.query(ArticleContent).filter_by(article_id=article.id).first()
            if existing_content:
                print(f"  ⚠️ Article '{article.title[:50]}...' already has content, skipping")
                continue
            ...
            print(f"     Extracted: {content_extracted_at.strftime('%Y-%m-%d %H:%M UTC')}")
...
def run_ttl_demo():
    ...
    print(f"   Duration: {ttl_info['ttl_hours']} hours ({ttl_info['ttl_hours']//24} days)")
    ...
    print(f"   Content Created: {stats_after_creation['total_records'] - stats_initial['total_records']}")
    ...
    print("   • Monitor storage with: python -c \"from app.jobs.ttl_cleanup import get_content_statistics; print(get_content_statistics())\"")
