...
def run_ingestion(include_crawling: bool = True, max_crawl_jobs: int = 5) -> None:
    ...
    if include_crawling:
        ...
        if crawl_result['successful'] > 0:
            logging.info("✅ Successfully extracted content from %d articles", crawl_result['successful'])
        
        if crawl_result['failed'] > 0:
            logging.info("⚠️ %d crawl jobs failed (may be rate-limited for retry)", crawl_result['failed'])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--no-crawl":
            ...
            logging.info("✅ Crawl worker completed: %d successful, %d failed", 
