...
def main():
    ...
    try:
        articles = (
            db.query(Article).order_by(Article.published_at.desc()).limit(5).all()
        )
        ...
        for a in articles:
            sentiment_icon = (
                "😊"
                if str(a.sentiment_label) == "positive"
                else "😔" if str(a.sentiment_label) == "negative" else "😐"
            )
            print(
                f"{sentiment_icon} {a.published_at} | {a.sentiment_label} ({a.sentiment_score})"
            )
            print(f"   📰 {a.title}")
            print()
