...
| 3.E1 | Elevation of privilege | A regular user becomes admin. | Admin capability (e.g. `POST /sources/`) is gated by `require_admin` ([app/deps/auth.py](../app/deps/auth.py)), which reads a `role` claim from the Firebase ID token. The claim is set only via the Admin SDK and is part of the Google-signed token, so it is verified on every request and cannot be forged client-side. Machine endpoints stay separately gated by OIDC (4.S1) regardless of any Firebase claim. | Claims are set out-of-band via the Admin SDK (a privileged op, not user-settable); forging a role would require compromising Firebase's signing key (3.S1). There is no in-app admin-management UI, so over-provisioning is an operational rather than a code risk. |
...
| 6.I1 | Information disclosure | Storing full article bodies creates a copyright + privacy liability. | Content truncated to 1024 chars + 7-day TTL (`config.ingestion.article_content_ttl_hours`). The deployed cleanup path is `full_database_cleanup` in `app/utils/cleanup.py`, invoked by the OIDC-protected `POST /api/v1/cleanup` endpoint that Cloud Scheduler runs daily. (`app/jobs/ttl_cleanup.py` is a standalone CLI/test helper, not the production path.) | None — truncation + TTL is the design. |
